...
export const authorResolvers = {
  ...
  findAuthorByBook: async (bookId: string) => {
    try {
      const author = await Author.findOne({ books: new mongoose.Types.ObjectId(bookId) });
      if (!author) {
  ...
  findAuthorByName: async (firstName: string, lastName?: string) => {
    ...
    addAuthor: async ({ firstName, lastName, bio, birthYear, deathYear, bookIds = []  }: { 
        firstName: string, 
        lastName: string, 
        bio?: string, 
        birthYear?: number, 
    ...
    addBookToAuthor: async ({ authorId, bookId }: {
        authorId: string,
        bookId: string,
    }) => {
        try {
    ...
    removeBookFromAuthor: async ({ authorId, bookId }: {
        authorId: string,
        bookId: string,
    }) => {
        try {
